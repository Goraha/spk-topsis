...
<div class="container">
  <div class="row">
    <div class="col-lg-12" align="center">
      <h1><span class="fa fa-users"></span> Ubah Data Alternatif</h1>
    </div>
  </div>
  <div class="row">
        ...
        <div class="form-group">
          <div class="row">
            <div class="col-md-3" align="left">
              ...
              <input type="text" name="kd_alternatif" id="kd_alternatif" value="<?php echo $kd_alternatif; ?>" class="form-control" autocomplete="off" required>
            </div>
            <div class="col-md-3" align="left">
              <label>Kode Anggota</label>
              <div class="input-group mb-3">
                <input type="text" name="nba" class="form-control" id="nba" value="<?php echo $data['nba']; ?>" autocomplete="off" required>
            ...
            <div class="col-md-6" align="left">
              ...
              <input type="text" name="nm_lengkap" id="nm_lengkap" class="form-control" value="<?php echo $data['nm_lengkap']; ?>" autocomplete="off" required>
            </div>
          </div>
...
$query = "UPDATE tbl_alternatif SET kd_alternatif='".$kd_alternatif."', nba='".$nba."' WHERE kd_alternatif='".$kd_awal."'";
$sql = mysqli_query($connect, $query);
    if($sql){
//echo "<script language=\"javascript\">alert(\"$n\");</script>";
    for($i=0; $i < $n; $i++){
        $b=$arr[$i];
        $sql3 = $pdo->prepare("SELECT * FROM tbl_subkriteria WHERE kd_subkriteria='$b'");
        $sql3->execute();
        while($data3 = $sql3->fetch()){
        $c= $data3['kd_kriteria'];
        $cek = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tbl_nilai_alternatif as a left join tbl_subkriteria as b on a.kd_subkriteria=b.kd_subkriteria left join tbl_kriteria as c on b.kd_kriteria=c.kd_kriteria where a.kd_alternatif= '$kd_alternatif' and c.kd_kriteria ='$c'"));
        
            IF($cek > 0){
            $query = "DELETE a FROM tbl_nilai_alternatif as a left join tbl_subkriteria as b on a.kd_subkriteria=b.kd_subkriteria left join tbl_kriteria as c on b.kd_kriteria=c.kd_kriteria WHERE kd_alternatif= '$kd_alternatif' and c.kd_kriteria ='$c'";
            //echo "<script language=\"javascript\">alert(\"$query\")</script>";
            $sql = mysqli_query($connect, $query);
            $query7 = "INSERT INTO tbl_nilai_alternatif VALUES('','$kd_alternatif','$b')";
            $sql7 = mysqli_query($connect, $query7);
            }else{
            $query = "INSERT INTO tbl_nilai_alternatif VALUES('','$kd_alternatif','$b')";
            $sql = mysqli_query($connect, $query);
            }
        
        }
    }
        echo "<script language=\"javascript\">alert(\"Berhasil Mengubah Data\");document.location.href='data_alternatif.php';</script>";
    }else{
        echo "<script language=\"javascript\">alert(\"Gagal Mengubah Data\");document.location.href='data_alternatif.php';</script>";
    }
}
?>